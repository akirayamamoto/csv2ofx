...
def ynab_toOFXDate(date):
    return datetime.strptime(date, '%d/%m/%Y').strftime('%Y%m%d')

def ynab_toQIFDate(date):
    return datetime.strptime(date, '%d/%m/%Y').strftime('%m/%d/%Y')

def ynab_toAmount(debit, credit):
    amount = 0
    if debit:
      amount -= float(debit)
    if credit:
...
def ynab_transactionId(date, amount, payee):
    return str(binascii.crc32(json.dumps([date, amount, payee])))
    # return str(zlib.crc32(json.dumps([date, amount, payee])))

ynab = {
    'OFX':{
        'skip':lambda row, grid: False,
        'BANKID':lambda row, grid: 'YNAB Bank',
        'ACCTID':lambda row, grid: 'YNAB Account',
        'DTPOSTED':lambda row, grid: ynab_toOFXDate(fromCSVCol(row, grid, 'Date')),
        'TRNAMT':lambda row, grid: ynab_toAmount(fromCSVCol(row, grid, 'Outflow'), fromCSVCol(row, grid, 'Inflow')),
        'FITID':lambda row, grid: ynab_transactionId(ynab_toOFXDate(fromCSVCol(row, grid, 'Date')),
                                                     ynab_toAmount(fromCSVCol(row, grid, 'Outflow'), fromCSVCol(row, grid, 'Inflow')),
                                                     fromCSVCol(row, grid, 'Payee')),
        'PAYEE':lambda row, grid: fromCSVCol(row, grid, 'Payee'),
        'MEMO':lambda row, grid: fromCSVCol(row, grid, 'Memo'),
        'CURDEF':lambda row, grid: 'BRL',
        'CHECKNUM':lambda row, grid: ynab_transactionId(ynab_toOFXDate(fromCSVCol(row, grid, 'Date')),
                                                        ynab_toAmount(fromCSVCol(row, grid, 'Outflow'), fromCSVCol(row, grid, 'Inflow')),
                                                        fromCSVCol(row, grid, 'Payee'))
    },
    'QIF':{
        'split':lambda row, grid:False,
        'Account':lambda row, grid: 'YNAB Account',
        'AccountDscr':lambda row, grid: 'YNAB Account Description',
        'Date':lambda row, grid: ynab_toQIFDate(fromCSVCol(row, grid, 'Date')),
        'Payee':lambda row, grid: fromCSVCol(row, grid, 'Payee'),
        'Memo':lambda row, grid: fromCSVCol(row, grid, 'Memo'),
        'Category':lambda row, grid:'',
        'Class':lambda row, grid:'',
        'Amount':lambda row, grid: ynab_toAmount(fromCSVCol(row, grid, 'Outflow'), fromCSVCol(row, grid, 'Inflow')),
        'Number':lambda row, grid: ynab_transactionId(ynab_toOFXDate(fromCSVCol(row, grid, 'Date')),
                                                      ynab_toAmount(fromCSVCol(row, grid, 'Outflow'), fromCSVCol(row, grid, 'Inflow')),
                                                      fromCSVCol(row, grid, 'Payee'))
    }
}
